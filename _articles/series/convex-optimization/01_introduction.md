...
また，等式制約 $$h_j(x)=0$$ は，その等式を満たす点の集合を表す．
特に $$a^T x=b$$ のようなアフィンな等式制約では，2次元では直線，3次元では平面，高次元では超平面となる．
...
今回の例では集合を二次元的な領域で表している．

領域以外にも，凸性は関数にも定義できる．
関数の凸性は，関数が描くグラフ上の任意の2点を結ぶ線分が，その間のグラフよりも上にあるかどうかで判断できる．
下に凸の二次関数をイメージすると，
二次関数上の任意の2点を結ぶ線分は，その間の二次関数のグラフよりも常に上にあり，凸関数である．
一方，四次関数のようにグラフに複数の山や谷がある場合，
任意の2点を結ぶ線分が，その間のグラフの山部分よりも下にある場合があり，非凸関数となる．

これらは，変数が増えても同じことが言える．
